...
//
// Set PLAID_REDIRECT_URI to 'http://localhost:3003'
...
app.post('/api/info', function (request, response, next) {
  ...
  const platformFlag = url.searchParams.get("redirect_uri") !== null;
  console.log("the Platform Flag code",platformFlag);
  response.json({
    item_id: ITEM_ID,
...
app.post('/api/send_the_access_token_AUTH', function (request, response, next) {
  PUBLIC_TOKEN = request.body.public_token || request.body.code;
  Promise.resolve()
    .then(async function () {
      const tokenResponse = await client.itemPublicTokenExchange({
        public_token: PUBLIC_TOKEN,
      ...
      return response.json({ closeWindow: true, access_token: ACCESS_TOKEN });
    })
    .catch(next);
});
